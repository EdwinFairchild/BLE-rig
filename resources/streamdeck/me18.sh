...
echo $currentState
keyValuePairs=$(/usr/bin/python3 -c "import sys, json; fileDict=json.load(open('$FILE')); fileDict['me18']['state'] = $currentState; file = open('$FILE', 'w'); json.dump(fileDict,file,indent=4);file.close();print(fileDict)")
echo $keyValuePairs
date
