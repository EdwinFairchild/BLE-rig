...
*       -DM_AUTH_MITM_FLAG  : MITM (authenticated pairing) requested
                              pairing method is determined by IO capabilities
below
*
 ...
 *   Pairing Request (SMP_CMD_PAIR_REQ) packets and Pairing Response
 * (SMP_CMD_PAIR_RSP) packets when the MITM flag is set in Configurable security
 * parameters above. -SMP_IO_DISP_ONLY         : Display only.
...
/* iOS connection parameter update requirements
 ...
 The connection parameter request may be rejected if it does not meet the
 ...
 * One of the following:
   ...
   * Supervision Timeout greater than Interval Max * (Peripheral Latency + 1)
 * 3.
*/
...
/*! client characteristic configuration descriptors settings, indexed by above
 * enumeration */
...
static void datsCccCback(attsCccEvt_t *pEvt) {
  ...
  /* If CCC not set from initialization and there's a device record and
   * currently bonded */
 ...
 *  \details 32 kHz crystal will drift over temperature, execute this trim
 * procedure to align with 32 MHz clock. System will not be able to enter
 * standby mode while this procedure is in progress (200-600 ms). Larger period
 * values will save power. Do not execute this procedure while BLE hardware is
 * disabled. Disable this periodic trim if under constant temperature. Refer to
 * 32 kHz crystal data sheet for temperature variance.
 *
 *  \return  None.
 */
/*************************************************************************************************/
...
/*************************************************************************************************/
/*!
 *  \brief  Set up advertising and other procedures that need to be performed
 * after device reset.
 *
...
static void datsSetup(dmEvt_t *pMsg) {
  ...
  AppAdvSetData(APP_ADV_DATA_DISCOVERABLE, sizeof(datsAdvDataDisc),
                (uint8_t *)datsAdvDataDisc);
  AppAdvSetData(APP_SCAN_DATA_DISCOVERABLE, sizeof(datsScanDataDisc),
                (uint8_t *)datsScanDataDisc);

  /* set advertising and scan response data for connectable mode */
  AppAdvSetData(APP_ADV_DATA_CONNECTABLE, sizeof(datsAdvDataDisc),
                (uint8_t *)datsAdvDataDisc);
  AppAdvSetData(APP_SCAN_DATA_CONNECTABLE, sizeof(datsScanDataDisc),
                (uint8_t *)datsScanDataDisc);
...
static void datsProcMsg(dmEvt_t *pMsg) {
  ...
  case DM_CONN_CLOSE_IND:
    ...
    APP_TRACE_INFO2("Connection closed status 0x%x, reason 0x%x",
                    pMsg->connClose.status, pMsg->connClose.reason);
...
/*************************************************************************************************/
/*!
 *  \brief  Application handler init function called during system
 * initialization.
 *
