...
static const appDiscCfg_t datcDiscCfg = {
    FALSE, /*! TRUE to wait for a secure connection before initiating discovery
            */
    TRUE   /*! TRUE to fall back on database hash to verify handles when no bond
              exists. */
};
...
static const attcDiscCfg_t datcDiscCfgList[] = {
    ...
    {datcCccIndVal, sizeof(datcCccIndVal),
     (GATT_SC_CCC_HDL_IDX + DATC_DISC_GATT_START)},
    ...
    {datcCccNtfVal, sizeof(datcCccNtfVal),
     (WPC_P1_NA_CCC_HDL_IDX + DATC_DISC_WP_START)}};
...
static void datcPrintScanReport(dmEvt_t *pMsg) {
  ...
  WsfTrace("  %02x:%02x:%02x:%02x:%02x:%02x", pMsg->scanReport.addr[5],
           pMsg->scanReport.addr[4], pMsg->scanReport.addr[3],
           pMsg->scanReport.addr[2], pMsg->scanReport.addr[1],
...
static void datcScanReport(dmEvt_t *pMsg) {
  ...
  if ((dbHdl = AppDbFindByAddr(pMsg->scanReport.addrType,
                               pMsg->scanReport.addr)) != APP_DB_HDL_NONE) {
    /* if this is a directed advertisement where the initiator address is an RPA
     */
    if (DM_RAND_ADDR_RPA(pMsg->scanReport.directAddr,
                         pMsg->scanReport.directAddrType)) {
      /* resolve direct address to see if it's addressed to us */
      AppMasterResolveAddr(pMsg, dbHdl, APP_RESOLVE_DIRECT_RPA);
    } else {
      connect = TRUE;
    }
  } else if (DM_RAND_ADDR_RPA(pMsg->scanReport.addr,
                              pMsg->scanReport.addrType)) {
    /* if the peer device uses an RPA */
    /* resolve advertiser's RPA to see if we already have a bond with this
     * device */
...
static void datcValueNtf(attEvt_t *pMsg) {
  if (pMsg->handle == pDatcWpHdlList[pMsg->hdr.param - 1][WPC_P1_DAT_HDL_IDX])
...
static void datcSendData(dmConnId_t connId) {
  ...
  if (pDatcWpHdlList[connId - 1][WPC_P1_DAT_HDL_IDX] != ATT_HANDLE_NONE) {
    AttcWriteCmd(connId, pDatcWpHdlList[connId - 1][WPC_P1_DAT_HDL_IDX],
                 sizeof(powerOptions_t), &temp);
  }
}
...
static void datcBtnCback(uint8_t btn) {
  ...
  if (numConnections > 0) {
    ...
    case APP_UI_BTN_1_EX_LONG:
      /* add RPAO characteristic to GAP service -- needed only when DM Privacy
       * enabled */
      SvcCoreGapAddRpaoCh();
      break;
