...
def btnAllOn(interface):
    Slots.set_device_power_settings(interface, "me17_main", True)
    Slots.set_device_power_settings(interface, "me17", True)
    Slots.set_device_power_settings(interface, "me14", True)
    Slots.set_device_power_settings(interface, "me18", True)


def btnAllOff(interface):
    Slots.set_device_power_settings(interface, "me17_main", False)
    Slots.set_device_power_settings(interface, "me17", False)
    Slots.set_device_power_settings(interface, "me14", False)
    Slots.set_device_power_settings(interface, "me18", False)
...
def register_button_callbacks(interface):
    ...
    interface.ui.btn_all_off.clicked.connect(
        lambda state: btnAllOff(interface))
